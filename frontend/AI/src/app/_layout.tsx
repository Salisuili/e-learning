...
function RootLayoutContent() {
  ...
  useEffect(() => {
    ...
    if (user.role !== 'admin' && (!user.is_approved || user.status === 'pending' || user.status === 'rejected')) {
      if (!isPendingApproval && !isPublicRoute) {
        router.replace('/pending-approval');
      }
      return;
    }

    // If user is approved and on pending page, redirect to dashboard
    if (isPendingApproval && (user.is_approved || user.status === 'approved')) {
    ...
    if (isPublicRoute) {
      // Redirect logged-in approved users away from public pages to their dashboard
      if (role === 'student') {
        router.replace('/(student)/dashboard');
